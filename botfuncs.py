...
async def ping_random_user(thread) -> None:
    ...
    if not users:
        self.add_task(ping_random_user, datetime.now() + timedelta(seconds = 15), arguments=(thread,))
        return

    username = random.choice(users).username
    message = await thread.bot_instance.send_message(thread.chat_instance.id, f'@{username}')

    thread.add_task(ping_random_user, datetime.now() + timedelta(minutes = cfg.randping_delay), arguments=(thread,))
